...
/** HK SME-FRS-style expense nature classes (management accounts). */
...
const AI_PROMPT = `請分析這張香港餐廳／快餐店收據，以 JSON 格式回傳：
- merchant_name (商戶名稱)
- amount (總金額, 數字, HKD)
- expense_date (日期 YYYY-MM-DD)
- category (必須係以下其中一個英文 key)：
  cost_of_sales = 食材／飲料／原料（銷售成本）
  packaging = 包裝／外賣盒／紙袋
  rent_utilities = 租金／水電煤／管理費／網絡電話
  repairs = 維修保養／設備零件
  marketing = 廣告推廣／優惠券／社交媒體
  admin = 文具／會計／銀行手續／保險／牌照
  other = 其他營運開支
只回傳 JSON，不要其他文字。`;
...
function normalizeParsed(parsed) {
    ...
    return {
        ...
        expense_date,
        category: normalizeCategory(parsed?.category || parsed?.expense_category || parsed?.type),
    };
}
...
function geminiVisionModels() {
    ...
    for (const raw of models) {
        const model = String(raw || '').trim().replace(/^models\//, '');
        if (!model) continue;
        // Retired Gemini 2.x — never call even if env still points here.
        if (/^gemini-2(\.|$)/i.test(model)) continue;
...
async function analyzeWithGemini(base64, mime) {
    ...
    for (const model of geminiVisionModels()) {
        ...
        const parsed = extractJson(text);
        if (!parsed) {
            lastErr = new Error('Gemini returned non-JSON');
            lastErr.status = 502;
            continue;
        }
...
async function insertExpense(row) {
    ...
    let { resp, data } = await post(row);
    // Category column may not exist until expenses-categories.sql is applied.
    if (!resp.ok && row.category != null) {
        const msg = typeof data === 'string' ? data : (data.message || JSON.stringify(data));
        if (/category|notes|schema cache|PGRST/i.test(msg)) {
            const fallback = { ...row };
            delete fallback.category;
            delete fallback.notes;
            ({ resp, data } = await post(fallback));
        }
    }
    if (!resp.ok) {
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        let aiError = null;
        try {
            // Prefer Gemini for Mirror Burger (OPENAI optional later).
            ai = await analyzeWithGemini(base64, mime);
            if (!ai) ai = await analyzeWithOpenAI(base64, mime);
            if (!ai) {
                const err = new Error('Missing GEMINI_API_KEY (or OPENAI_API_KEY)');
                err.status = 500;
                throw err;
            }
        } catch (err) {
