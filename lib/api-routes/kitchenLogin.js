...
const {
    ...
    enrichAuthClaims,
} = require('../../api/_kitchenAuth.js');
const { mintKitchenSupabaseJwt, getJwtSecret } = require('../../api/_supabaseJwt.js');
const {
    MASTER_ID,
    passwordMatches,
    saveHash,
    validateNewPassword,
    accountIdFor,
} = require('../../api/_kitchenPasswords.js');
...
function sessionPayload(auth) {
    const enriched = enrichAuthClaims(auth);
    const supabase_access_token = mintKitchenSupabaseJwt(enriched);
    return {
        ok: true,
        token: makeKitchenToken(enriched),
        scope: enriched.scope,
        store_name: enriched.store_name || '',
        store_id: enriched.store_id || '',
        supabase_access_token: supabase_access_token || null,
        supabase_jwt_ready: Boolean(getJwtSecret() && supabase_access_token),
    };
}
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (body.token && !body.password) {
            ...
            if (auth) {
                return res.status(200).json(sessionPayload(auth));
            }
