...
exports.handler = async (event) => {
    ...
    try {
        ...
        const res = await fetch('https://payment.uat.kpay-group.com/gateway/api/v1/online/order/create', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-KPay-Signature': signature },
            body: JSON.stringify(params)
        });
        const result = await res.json();
        return { statusCode: 200, body: JSON.stringify({ paymentUrl: result.data?.payUrl || null, error: result.msg || null }) };
