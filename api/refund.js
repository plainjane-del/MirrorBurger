...
// 💡 呢度我將你嘅 Key 拆開處理，確保無任何隱藏字符影響解碼
const PRIVATE_KEY_DATA = [
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!orderNo || !amount) return { statusCode: 400, body: JSON.stringify({ error: "缺少單號或金額" }) };

        const nonceStr = Math.random().toString(36).substring(2, 15);
        const timestamp = Date.now().toString();

        // 1. 構建簽名原始字串 (KPay v4.0 要求按字母排序，每行 key=value，最後一行必須有 \n)
        const signParams = {
        ...
        sign.update(signStr);
        // 使用更加兼容嘅方式來加載 Private Key
        const signature = sign.sign({
            key: PRIVATE_KEY,
            padding: crypto.constants.RSA_PKCS1_PADDING
        }, 'base64');

        // 3. 呼叫 KPay UAT 退款接口 (/v1/refund)
        const bodyData = {
            outTradeNo: "REF" + Date.now(), // 商家自定義退款單號
            oriOrderNo: orderNo,            // 原始交易單號
