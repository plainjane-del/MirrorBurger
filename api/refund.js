...
exports.handler = async (event) => {
    ...
    try {
        ...
        const res = await fetch('https://payment.uat.kpay-group.com/gateway/api/v1/online/order/refund', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-KPay-Signature': signature },
            body: JSON.stringify(params)
        });
        const result = await res.json();
        if (result.code === 'SUCCESS') return { statusCode: 200, body: JSON.stringify({ success: true }) };
        return { statusCode: 400, body: JSON.stringify({ success: false, error: result.msg }) };
